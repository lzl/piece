...
class CloneBoxComponent extends Component {
  render() {
    if (this.props.cloneNum > 1 || this.props.enableCloneFeature) {
      return (
        <div>
          <CloneNewBox />
          <div className="hr" />
...
class CloneSelectBoxComponent extends Component {
  render () {
    if (this.props.clones.length > 1) {
      return (
        <select className="c-select" ref="select" defaultValue={this.props.currentClone._id} onChange={this.handleSelect.bind(this)}>
...
CloneSelectBox = createContainer(() => {
  P.setCurrentCloneId();
  const currentCloneId = Session.get("currentCloneId");
  return {
